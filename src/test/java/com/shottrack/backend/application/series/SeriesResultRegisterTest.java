package com.shottrack.backend.application.series;

import com.fasterxml.jackson.databind.JsonNode;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.shottrack.backend.application.modality.dto.AddModalityResultTypeRequest;
import com.shottrack.backend.application.series.dto.MarkSeriesResultNotApplicableRequest;
import com.shottrack.backend.application.series.dto.RegisterSeriesRequest;
import com.shottrack.backend.application.series.dto.RegisterSeriesResultRequest;
import com.shottrack.backend.application.user.gateway.repository.PendingRegistrationRepository;
import com.shottrack.backend.support.SeriesTestSupport;
import com.shottrack.backend.support.TestUsers;
import org.junit.jupiter.api.Test;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.autoconfigure.web.servlet.AutoConfigureMockMvc;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.http.MediaType;
import org.springframework.test.web.servlet.MockMvc;
import org.springframework.transaction.annotation.Transactional;

import java.util.UUID;

import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.get;
import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.post;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.jsonPath;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.status;

@SpringBootTest
@AutoConfigureMockMvc
@Transactional
class SeriesResultRegisterTest {

    private static final String PASSWORD = "senha12345";

    @Autowired
    private MockMvc mockMvc;

    @Autowired
    private ObjectMapper objectMapper;

    @Autowired
    private PendingRegistrationRepository pendingRegistrationRepository;

    @Test
    void shouldRegisterNumericValue() throws Exception {
        String token = registerAndLogin("atleta.resultadonumerico@shottrack.com");
        UUID trainingId = SeriesTestSupport.openTraining(mockMvc, objectMapper, token, "IPSC");
        UUID seriesId = registerSeries(token, trainingId);
        UUID tempoId = resultTypeIdByName(token, "Tempo");

        mockMvc.perform(post("/api/series/" + seriesId + "/results")
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new RegisterSeriesResultRequest(tempoId, "12.34"))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.resultTypeName").value("Tempo"))
                .andExpect(jsonPath("$.data.value").value("12.34"))
                .andExpect(jsonPath("$.data.notApplicable").value(false));
    }

    @Test
    void shouldRegisterZeroValueDistinctFromUnfilled() throws Exception {
        String token = registerAndLogin("atleta.resultadozero@shottrack.com");
        UUID trainingId = SeriesTestSupport.openTraining(mockMvc, objectMapper, token, "IPSC");
        UUID seriesId = registerSeries(token, trainingId);
        UUID pontuacaoId = resultTypeIdByName(token, "Pontuação");

        mockMvc.perform(post("/api/series/" + seriesId + "/results")
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new RegisterSeriesResultRequest(pontuacaoId, "0"))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.value").value("0"))
                .andExpect(jsonPath("$.data.notApplicable").value(false));

        mockMvc.perform(get("/api/trainings/" + trainingId + "/series")
                        .header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data[0].results.length()").value(1))
                .andExpect(jsonPath("$.data[0].results[0].value").value("0"));
    }

    @Test
    void shouldMarkAsNotApplicable() throws Exception {
        String token = registerAndLogin("atleta.resultadonaoaplicavel@shottrack.com");
        UUID trainingId = SeriesTestSupport.openTraining(mockMvc, objectMapper, token, "IPSC");
        UUID seriesId = registerSeries(token, trainingId);
        UUID fatorId = resultTypeIdByName(token, "Fator de desempenho");

        mockMvc.perform(post("/api/series/" + seriesId + "/results/not-applicable")
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new MarkSeriesResultNotApplicableRequest(fatorId))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.notApplicable").value(true))
                .andExpect(jsonPath("$.data.value").isEmpty());
    }

    @Test
    void shouldReplaceExistingValue() throws Exception {
        String token = registerAndLogin("atleta.substituivalor@shottrack.com");
        UUID trainingId = SeriesTestSupport.openTraining(mockMvc, objectMapper, token, "IPSC");
        UUID seriesId = registerSeries(token, trainingId);
        UUID tempoId = resultTypeIdByName(token, "Tempo");

        registerValue(token, seriesId, tempoId, "10");

        mockMvc.perform(post("/api/series/" + seriesId + "/results")
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new RegisterSeriesResultRequest(tempoId, "20"))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.value").value("20"));

        mockMvc.perform(get("/api/trainings/" + trainingId + "/series")
                        .header("Authorization", "Bearer " + token))
                .andExpect(jsonPath("$.data[0].results.length()").value(1));
    }

    @Test
    void shouldReplaceNotApplicableWithValue() throws Exception {
        String token = registerAndLogin("atleta.substituinaoaplicavel@shottrack.com");
        UUID trainingId = SeriesTestSupport.openTraining(mockMvc, objectMapper, token, "IPSC");
        UUID seriesId = registerSeries(token, trainingId);
        UUID tempoId = resultTypeIdByName(token, "Tempo");

        mockMvc.perform(post("/api/series/" + seriesId + "/results/not-applicable")
                .header("Authorization", "Bearer " + token)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(new MarkSeriesResultNotApplicableRequest(tempoId))));

        mockMvc.perform(post("/api/series/" + seriesId + "/results")
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new RegisterSeriesResultRequest(tempoId, "15"))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.value").value("15"))
                .andExpect(jsonPath("$.data.notApplicable").value(false));
    }

    @Test
    void shouldRegisterBooleanValue() throws Exception {
        String token = registerAndLogin("atleta.resultadobooleano@shottrack.com");
        UUID trainingId = SeriesTestSupport.openTraining(mockMvc, objectMapper, token, "IPSC");
        UUID modalityId = modalityIdByName(token, "IPSC");
        UUID seriesId = registerSeries(token, trainingId);
        UUID exercicioId = configureResultType(token, modalityId, "Exercício concluído");

        mockMvc.perform(post("/api/series/" + seriesId + "/results")
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new RegisterSeriesResultRequest(exercicioId, "sim"))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.value").value("sim"));
    }

    @Test
    void shouldRejectInvalidBooleanValue() throws Exception {
        String token = registerAndLogin("atleta.resultadobooleaninvalido@shottrack.com");
        UUID trainingId = SeriesTestSupport.openTraining(mockMvc, objectMapper, token, "IPSC");
        UUID modalityId = modalityIdByName(token, "IPSC");
        UUID seriesId = registerSeries(token, trainingId);
        UUID exercicioId = configureResultType(token, modalityId, "Exercício concluído");

        mockMvc.perform(post("/api/series/" + seriesId + "/results")
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new RegisterSeriesResultRequest(exercicioId, "talvez"))))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error.code").value("RESULT_VALUE_FORMAT_INVALID"));
    }

    @Test
    void shouldRegisterFreeTextValue() throws Exception {
        String token = registerAndLogin("atleta.resultadotextolivre@shottrack.com");
        UUID trainingId = SeriesTestSupport.openTraining(mockMvc, objectMapper, token, "IPSC");
        UUID modalityId = modalityIdByName(token, "IPSC");
        UUID seriesId = registerSeries(token, trainingId);
        UUID anotacaoId = configureResultType(token, modalityId, "Anotação livre");

        mockMvc.perform(post("/api/series/" + seriesId + "/results")
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new RegisterSeriesResultRequest(anotacaoId, "Ombro doendo hoje"))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.value").value("Ombro doendo hoje"));
    }

    @Test
    void shouldRejectRequestWithoutToken() throws Exception {
        mockMvc.perform(post("/api/series/" + UUID.randomUUID() + "/results")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new RegisterSeriesResultRequest(UUID.randomUUID(), "10"))))
                .andExpect(status().isUnauthorized())
                .andExpect(jsonPath("$.error.code").value("UNAUTHORIZED"));
    }

    @Test
    void shouldRejectNonExistentSeries() throws Exception {
        String token = registerAndLogin("atleta.resultadoserieinexistente@shottrack.com");

        mockMvc.perform(post("/api/series/" + UUID.randomUUID() + "/results")
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new RegisterSeriesResultRequest(UUID.randomUUID(), "10"))))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.error.code").value("SERIES_NOT_FOUND"));
    }

    @Test
    void shouldRejectRegisteringOnAnotherAthletesSeries() throws Exception {
        String tokenDono = registerAndLogin("atleta.donoserieresultado@shottrack.com");
        String tokenOutro = registerAndLogin("atleta.naoedonoserieresultado@shottrack.com");
        UUID trainingId = SeriesTestSupport.openTraining(mockMvc, objectMapper, tokenDono, "IPSC");
        UUID seriesId = registerSeries(tokenDono, trainingId);
        UUID tempoId = resultTypeIdByName(tokenDono, "Tempo");

        mockMvc.perform(post("/api/series/" + seriesId + "/results")
                        .header("Authorization", "Bearer " + tokenOutro)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new RegisterSeriesResultRequest(tempoId, "10"))))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.error.code").value("SERIES_NOT_FOUND"));
    }

    @Test
    void shouldRejectResultTypeNotConfiguredForTrainingModality() throws Exception {
        String token = registerAndLogin("atleta.tiponaoconfigurado@shottrack.com");
        UUID trainingId = SeriesTestSupport.openTraining(mockMvc, objectMapper, token, "IPSC");
        UUID seriesId = registerSeries(token, trainingId);
        UUID agrupamentoId = resultTypeIdByName(token, "Agrupamento");

        mockMvc.perform(post("/api/series/" + seriesId + "/results")
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new RegisterSeriesResultRequest(agrupamentoId, "10"))))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.error.code").value("RESULT_TYPE_NOT_CONFIGURED_FOR_TRAINING"));
    }

    @Test
    void shouldRejectIncompatibleValueFormat() throws Exception {
        String token = registerAndLogin("atleta.formatoincompativel@shottrack.com");
        UUID trainingId = SeriesTestSupport.openTraining(mockMvc, objectMapper, token, "IPSC");
        UUID seriesId = registerSeries(token, trainingId);
        UUID tempoId = resultTypeIdByName(token, "Tempo");

        mockMvc.perform(post("/api/series/" + seriesId + "/results")
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new RegisterSeriesResultRequest(tempoId, "não é número"))))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error.code").value("RESULT_VALUE_FORMAT_INVALID"));
    }

    private void registerValue(String token, UUID seriesId, UUID resultTypeId, String value) throws Exception {
        mockMvc.perform(post("/api/series/" + seriesId + "/results")
                .header("Authorization", "Bearer " + token)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(new RegisterSeriesResultRequest(resultTypeId, value))));
    }

    private UUID configureResultType(String token, UUID modalityId, String resultTypeName) throws Exception {
        UUID resultTypeId = resultTypeIdByName(token, resultTypeName);

        mockMvc.perform(post("/api/practiced-modalities/" + modalityId + "/result-types")
                .header("Authorization", "Bearer " + token)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(new AddModalityResultTypeRequest(resultTypeId))));

        return resultTypeId;
    }

    private UUID registerSeries(String token, UUID trainingId) throws Exception {
        var result = mockMvc.perform(post("/api/series")
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(
                                new RegisterSeriesRequest(trainingId, null, null, null, null, null, null))))
                .andReturn();

        return UUID.fromString(objectMapper.readTree(result.getResponse().getContentAsString()).get("data").get("id").asText());
    }

    private UUID modalityIdByName(String token, String name) throws Exception {
        return idByName(token, "/api/modality-catalog", name);
    }

    private UUID resultTypeIdByName(String token, String name) throws Exception {
        return idByName(token, "/api/result-type-catalog", name);
    }

    private UUID idByName(String token, String path, String name) throws Exception {
        var result = mockMvc.perform(get(path)
                        .header("Authorization", "Bearer " + token))
                .andReturn();

        JsonNode items = objectMapper.readTree(result.getResponse().getContentAsString()).get("data");
        for (JsonNode item : items) {
            if (item.get("name").asText().equals(name)) {
                return UUID.fromString(item.get("id").asText());
            }
        }
        throw new AssertionError("Item não encontrado no catálogo (" + path + "): " + name);
    }

    private String registerAndLogin(String email) throws Exception {
        return TestUsers.registerAndLogin(mockMvc, objectMapper, pendingRegistrationRepository, "Atleta Teste", email, PASSWORD);
    }
}
